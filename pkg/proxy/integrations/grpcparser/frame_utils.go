...
func PrintDataFrame(frame *http2.DataFrame) {
	//fmt.Println("payload of dataFrame: ", protoscope.Write(frame.Data()[5:], protoscope.WriterOptions{}))
	fmt.Println("payload of dataFrame: ", string(frame.Data()))
	fmt.Printf("streamEnded: %v \n", frame.StreamEnded())
}

func PrintHeadersFrame(frame *http2.HeadersFrame) {
	fmt.Println("This is a HEADERS frame")
	fmt.Printf("headersEnded: %v , streamEnded: %v \n", frame.HeadersEnded(), frame.StreamEnded())
