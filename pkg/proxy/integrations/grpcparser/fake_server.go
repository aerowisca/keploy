...
func (srv *transcoder) ProcessDataFrame(dataFrame *http2.DataFrame) error {
	...
	if dataFrame.StreamEnded() {
		...
		if srv.hook.GetDepsSize() == 0 {
			...
			panic("failed to mock the output for unrecorded outgoing grpc call")
		}

		grpcReq := srv.sic.FetchRequestForStream(id)

		fmt.Println("the length of the body of the response body is :", len(grpcReq.Body))
		...
		if mock == nil {
			//return fmt.Errorf("failed to mock the output for unrecorded outgoing grpc call as mock is nil")
			panic("failed to mock the output for unrecorded outgoing grpc call as mock is nil")
		}
