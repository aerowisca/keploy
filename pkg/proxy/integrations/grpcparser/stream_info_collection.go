...
func (sic *StreamInfoCollection) AddPayloadForRequest(streamID uint32, payload []byte) {
	...
	tempReqBody := sic.StreamInfo[streamID].GrpcReq.Body
	tempReqBody = append(tempReqBody, CreateLengthPrefixedMessageFromPayload(payload))
	info.GrpcReq.Body = tempReqBody
...
func (sic *StreamInfoCollection) AddPayloadForResponse(streamID uint32, payload []byte) {
	...
	// Create a copy and overwrite it.
	info := sic.StreamInfo[streamID]
	tempRespBody := sic.StreamInfo[streamID].GrpcResp.Body
	tempRespBody = append(tempRespBody, CreateLengthPrefixedMessageFromPayload(payload))
	info.GrpcResp.Body = tempRespBody
	sic.StreamInfo[streamID] = info
}
