...
func decodeOutgoingGRPC(requestBuffer []byte, clientConn, destConn net.Conn, h *hooks.Hook, logger *zap.Logger) {
	...
	if err != nil {
		logger.Error("could not server grpc request", zap.Error(err))
	}
}
	...
	for {
		...
		case *http2.HeadersFrame:
			...
			if err != nil {
				...
				// TODO : Fix the bug for parsing headers from second request onwards.
				fmt.Printf("Silently skipping header parsing error for now. error was %v\n", err)
			}
			...
			if isRespFromServer && headersFrame.StreamEnded() {
				fmt.Printf("stream ended: model: %+v\n", sic.StreamInfo[streamID])
